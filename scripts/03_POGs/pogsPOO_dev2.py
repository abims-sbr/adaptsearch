...
def makeOrthogroups(list_pairwises_allsp):
    ...
    for ortho_pair1 in list_pairwises_allsp[0:-1]:
        ...
        if len(orthogroup) != 0:
            # Code below is a bit messy ...

            # only one case : the first group added to the list
            if len(list_orthogroups) == 0:
                list_orthogroups.append(orthogroup)

            # general loop - checks for subsets ; if no subsets, proceeds to unions
            else:
                cond = issubgroup(orthogroup, list_orthogroups) # check for subsets - maybe not necessary ?
                if cond == False: 
                    for group in list_orthogroups :
                        if len(orthogroup.intersection(group)) != 0:
                            group.update(group | orthogroup)

                    if orthogroup not in list_orthogroups:
                        list_orthogroups.append(orthogroup)
            
    return list_orthogroups

def issubgroup(aset, listset):
    boole = False
    for curset in listset:        
        if aset.issubset(curset) :
            boole = True
            break

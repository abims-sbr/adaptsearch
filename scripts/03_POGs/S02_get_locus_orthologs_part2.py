...
## LAST VERSION : /01/2017 by Victor Mataigne and Gildas Le Corguille

# Command line : python get_locus_orthologs_part2.py <list_of_outputfiles_filter_assemblies> <integer:_minimal_number_seq_per_group> <paralogs_filtering:_yes/no>
...
import string, os, sys, pickle, re

## 1 ## LOAD "PICKLE" LIST OF ALL POTENTIAL LOCUS (orthologuous and paraloguous)
############################################################################################################################
file_LOCUS = open("02_backup_list_LOCUS") # output from part1, pickled.
...
while sp<len(L2) :
    sp+=1
    get_seq_per_locus(dico_LOCUS_sp['%i_sp' %sp], BASH, "outputs", "sp%i" %sp)        ## DEF4 ##

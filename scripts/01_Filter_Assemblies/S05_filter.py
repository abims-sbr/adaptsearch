...
if len(sys.argv) != 3:
    logging.error("Usage: S05_filter.py <input_fasta> <length_seq_max>")
    sys.exit(1)
...
logging.info(f"Filtering sequences in {path_IN} with max length {length_seq_max}")
...
if not sequences:
    ...
    with open(path_OUT, "w") as f_out:
        ...
        for record in sequences:
            if len(record.seq) <= length_seq_max:
